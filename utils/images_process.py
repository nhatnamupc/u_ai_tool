...
def generate_shadow_coordinates(im_shape, no_of_shadows, rectangular_roi, shadow_dimension):
    vertices_list = []
...
def shadow_process(image, no_of_shadows, x1, y1, x2, y2, shadow_dimension):
    ...
    vertices_list = generate_shadow_coordinates(im_shape, no_of_shadows, (x1, y1, x2, y2),
...
def add_shadow(image, no_of_shadows=1, rectangular_roi=(-1, -1, -1, -1),
    ...
    verify_image(image)
    if not (is_numeric(no_of_shadows) and 1 <= no_of_shadows <= 10):
        raise Exception(err_shadow_count)
    if not (is_numeric(shadow_dimension) and 3 <= shadow_dimension <= 10):
...
def generate_random_lines(im_shape, slant, drop_length, rain_type):
    drops = []
    ...
    for i in range(no_of_drops):  ## If You want heavy rain, try increasing this
        if slant < 0:
            x = np.random.randint(slant, im_shape[1])
        else:
            x = np.random.randint(0, im_shape[1] - slant)
        y = np.random.randint(0, im_shape[0] - drop_length)
        drops.append((x, y))
...
def add_rain(image, slant=-1, drop_length=20, drop_width=1, drop_color=(200, 200, 200),
    ...
    verify_image(image)
    slant_extreme = slant
    if not (is_numeric(slant_extreme) and (-20 <= slant_extreme <= 20) or slant_extreme == -1):
        raise Exception(err_rain_slant)
    if not (is_numeric(drop_width) and 1 <= drop_width <= 5):
        raise Exception(err_rain_width)
    if not (is_numeric(drop_length) and 0 <= drop_length <= 100):
    ...
    if is_list(image):
        ...
        rain_drops, drop_length = generate_random_lines(im_shape, slant, drop_length, rain_type)
        for img in image_list:
...
def generate_random_blur_coordinates(im_shape, hw):
    blur_points = []
    midx = im_shape[1] // 2 - 2 * hw
    midy = im_shape[0] // 2 - hw
    index = 1
    while midx > -hw or midy > -hw:
        for i in range(hw // 10 * index):
            x = np.random.randint(midx, im_shape[1] - midx - hw)
            y = np.random.randint(midy, im_shape[0] - midy - hw)
            blur_points.append((x, y))
        midx -= 3 * hw * im_shape[1] // sum(im_shape)
        midy -= 3 * hw * im_shape[0] // sum(im_shape)
        index += 1
    return blur_points
...
def add_fog(image, fog_coeff=-1):
    ...
    if is_list(image):
        ...
        for img in image_list:
            ...
            hw = int(im_shape[1] // 3 * fog_coeff_t)
            haze_list = generate_random_blur_coordinates(im_shape, hw)
...
def add_sun_flare_line(flare_center, angle, im_shape):
    x = []
    y = []
    i = 0
    for rand_x in range(0, im_shape[1], 10):
...
def add_sun_process(image, no_of_flare_circles, flare_center, src_radius, x, y, src_color):
    ...
    for i in range(no_of_flare_circles):
        ...
        rad = random.randint(1, im_shape[0] // 100 - 2)
    ...
    if is_list(image):
        ...
        for img in image_list:
            if angle == -1:
                ...
                angle_t = angle
            if flare_center == -1:
                flare_center_t = (random.randint(0, im_shape[1]), random.randint(0, im_shape[0] // 2))
            else:
                flare_center_t = flare_center
            x, y = add_sun_flare_line(flare_center_t, angle_t, im_shape)
...
def apply_motion_blur(image, count):
    image_t = image.copy()
    im_shape = image_t.shape
    size = 15
    kernel_motion_blur = np.zeros((size, size))
    kernel_motion_blur[int((size - 1) / 2), :] = np.ones(size)
    kernel_motion_blur = kernel_motion_blur / size
    i = im_shape[1] * 3 // 4 - 10 * count
    while i <= im_shape[1]:
        image_t[:, i:, :] = cv2.filter2D(image_t[:, i:, :], -1, kernel_motion_blur)
        image_t[:, :im_shape[1] - i, :] = cv2.filter2D(image_t[:, :im_shape[1] - i, :], -1, kernel_motion_blur)
        i += im_shape[1] // 25 - count
        count += 1
    image_RGB = image_t
    return image_RGB
...
def autumn_process(image):
    ...
    for i in range(0, im_shape[1], step):
        for j in range(0, im_shape[0], step):
            avg = np.average(image_hls[j:j + step, i:i + step, 0])
            #             print(avg)
            if 20 < avg < 100 and np.average(image[j:j + step, i:i + step, 1]) < 100:
...
def flip_h(image):  ##function to flip the image on horizontal axis
    verify_image(image)
...
def flip_v(image):  ##function to flip the image on vertical axis
    verify_image(image)
